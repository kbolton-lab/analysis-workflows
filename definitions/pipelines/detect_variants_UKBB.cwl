...
outputs:
    ...
    varscan_filtered_vcf:
        ...
        secondaryFiles: [.tbi]
    vardict_unfiltered_vcf:
        type: File
        outputSource: vardict/unfiltered_vcf
        secondaryFiles: [.tbi]
    vardict_filtered_vcf:
        type: File
        outputSource: vardict/filtered_vcf
        secondaryFiles: [.tbi]
...
steps:
    ...
    vardict:
        ...
        #<path_to_vardict_folder>/build/install/VarDict/bin/VarDict -G /path/to/hg19.fa -f $AF_THR -N tumor_sample_name -b "/path/to/tumor.bam|/path/to/normal.bam" -c 1 -S 2 -E 3 -g 4 /path/to/my.bed | VarDict/testsomatic.R | VarDict/var2vcf_paired.pl -N "tumor_sample_name|normal_sample_name" -f $AF_THR
            ...
            tumor_bam: tumor_bam
            normal_bam: normal_bam
            bed_targets: roi_intervals
            af_threshold: varscan_min_var_freq
            normal_sample_name: normal_sample_name
            tumor_sample_name: tumor_sample_name
        out:
            [unfiltered_vcf, filtered_vcf] # filtered applies bcbio filters
    pindel:
    ...
    #         [combined_vcf]
    sample_and_caller:
        run: ../tools/vcf_add_sample_name_and_caller_INFO.cwl
        in:
            reference: reference
            input_vcf: [mutect/filtered_vcf, varscan/filtered_vcf, vardict/filtered_vcf, pindel/filtered_vcf]
            caller: ["Mutect2", "Varscan2","Vardict","Pindel"]
        out:
            [renamed_vcf]
        scatter: [vcf_inputs, caller]
        scatterMethod: dotproduct
    combine:
        run: ../tools/combine_variants_UKBB.cwl
        in:
            reference: reference
            input_vcfs: sample_and_caller/renamed_vcf
            tumor_sample_name: tumor_sample_name
            # mutect_vcf: mutect/filtered_vcf
            # varscan_vcf: varscan/filtered_vcf
            # strelka_vcf: strelka/filtered_vcf
